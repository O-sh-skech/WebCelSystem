...
export const SpaceSliders: React.FC<SpaceSlidersProps> = ({ currentPart, values, onChange, sliderColor }) => {
      ...
      <div style={{
        ...
        {parts.map((part) => (
          <div key={part} style={{ width: `${100 / parts.length}%`, paddingLeft: '30px', paddingRight: '30px', boxSizing: 'border-box' }}>
            ...
            <SnapSlider label="Yaw (左右回転)" value={values.yaw} onChange={(val) => onChange('yaw', val)} sliderColor={sliderColor} />
            <SnapSlider label="Pitch (上下回転)" value={values.pitch} onChange={(val) => onChange('pitch', val)} sliderColor={sliderColor} />
            <SnapSlider label="Roll (首の傾き)" value={values.roll} onChange={(val) => onChange('roll', val)} sliderColor={sliderColor} />
          </div>
        ))}
