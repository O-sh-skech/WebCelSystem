...
CREATE TABLE IF NOT EXISTS cel_assets (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (parameter_id, part_id), -- 重複登録を完全に防ぐ複合主キー
    FOREIGN KEY (parameter_id) REFERENCES cel_parameters(id) ON DELETE CASCADE,
    FOREIGN KEY (part_id) REFERENCES parts(id) ON DELETE CASCADE -- ユーザid管理も見据えている
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
